# Foliograph Concept Index

**Project:** Scientific Discovery  
**Built:** 2026-05-26 23:42 UTC  
**Entries:** 20  

> Locate any concept, term, or section across all documents.
> Format: **Concept** → `file.ext § Section Title`
> To load: ask your LLM 'Load [file] § [Section]'

---


### A

- **Adatum Corporation** → `ScientificDiscovery.pptx § Slide 12`
- **Aidyn Zhanbolat** → `ScientificDiscovery.pptx § Slide 1`

### F

- **Financials Quarter Revenue** → `ScientificDiscovery.pptx`

### G

- **Growth Strategy Summary** → `ScientificDiscovery.pptx § Slide 2`

### M

- **Milestone Quarter Product** → `ScientificDiscovery.pptx`

### S

- **Slide 1** → `slide 1`
- **Slide 10** → `slide 10`
- **Slide 11** → `slide 11`
- **Slide 12** → `slide 12`
- **Slide 13** → `slide 13`
- **Slide 2** → `slide 2`
- **Slide 3** → `slide 3`
- **Slide 4** → `slide 4`
- **Slide 5** → `slide 5`
- **Slide 6** → `slide 6`
- **Slide 7** → `slide 7`
- **Slide 8** → `slide 8`
- **Slide 9** → `slide 9`
- **Summary With** → `ScientificDiscovery.pptx`

### T

- **Traction Product** → `ScientificDiscovery.pptx`
